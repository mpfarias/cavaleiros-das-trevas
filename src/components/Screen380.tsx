...
interface Screen380Props {
  onGoToScreen: (screenId: number) => void;
  ficha: Ficha;
  onUpdateFicha: (ficha: Ficha) => void;
}

const Screen380: React.FC<Screen380Props> = ({ onGoToScreen, ficha: _ficha, onUpdateFicha: _onUpdateFicha }) => {
  const { currentGroup, isPlaying, togglePlay } = useAudioGroup(380);
  const playClick = useClickSound(0.2);
  const theme = useScreenTheme(380);
  ...
  return (
    <Container data-screen="screen-380">
      <VolumeControl />
      ...
      <CardWrap>
        <CardContent sx={{ padding: '40px' }}>
          <NarrativeText>
            As três lâminas bem afiadas se fecham ao mesmo tempo.
          </NarrativeText>
